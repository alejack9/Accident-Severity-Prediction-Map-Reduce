...
//  private def printDs[T <: AnyVal: Fractional](D: Dataset[Seq[T]]): Unit = {
//    println("________Dataset_______")
//    println("| a_0  a_1  a_2    c |")
//    println("|--------------------|")
//    D.foreach{row => println("| % 3.0f  % 3.0f  % 3.0f  % 3.0f |".format(row(0), row(1), row(2), row(3)))}
//    println("----------------------")
//  }
  ...
  test("all ds | continuous | Double") {
    val (d, dtc) = getDouble
    val t = dtc.train(d, Vector(Format.Continuous, Format.Continuous, Format.Continuous))
    assert(t == CondNode(ContinuousCondition(0, 0.6666666666666666), Vector(
      Leaf(2.0), CondNode(ContinuousCondition(0, 5.0), Vector(
        Leaf(0.0), Leaf(2.0))))
    ).asInstanceOf[Tree[Float]])
  }
  ...
  test("all ds | categorical | Double") {
    val (d, dtc) = getDouble
    val t = dtc.train(d, Vector(Format.Categorical, Format.Categorical, Format.Categorical))
    assert(t == CondNode(CategoricalCondition(0, Vector(0.3333333333333333, 1.0, 3.0, 7.0)), Vector(
      Leaf(2.0),
      Leaf(0.0),
      Leaf(0.0),
      Leaf(2.0)
